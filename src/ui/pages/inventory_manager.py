...
def render():
    ...
    try:
        ...
        # Organize into Tabs
        tab1, tab2, tab3, tab4 = st.tabs(["📊 Current Balances", "➕ Update Stock", "📜 Recent History", "📥 Bulk Update"])
                ...
                if df_for_select.empty:
                    ...
                    with st.form("stock_form"):
                        ...
                        if filtered_row.empty:
                            ...
                            st.error(f"SKU {sku} not found in inventory. Please select a valid SKU.")
                        else:
                            current_row = filtered_row.iloc[0]
                            
                            # Safely convert pack_multiplier with fallback
                            try:
                        ...
                        if valid_rows == 0:
                            ...
                            if st.button("✅ Import Data", type="primary"):
                                try:
                                    ...
                                    df_upload['godown_stock_packs'] = pd.to_numeric(df_upload['godown_stock_packs'], errors='coerce').fillna(0).astype(int)
                                    df_upload['pack_multiplier'] = pd.to_numeric(df_upload['pack_multiplier'], errors='coerce').fillna(1).astype(int)
                                    ...
                                    try:
                                        for idx, row in df_upload.iterrows():
                                            ...
                                            # Check if SKU exists
                                            cursor.execute("SELECT COUNT(*) FROM inventory_master WHERE sku = ?", (sku,))
                                            ...
                                            if not exists:
                                                ...
                                                cursor.execute("""
                                                    ...
                                                    SET godown_stock_packs = ?, pack_multiplier = ?
                                                    WHERE sku = ?
                                                """, (packs, multiplier, sku))
                                                updated_count += 1
                                                logger.debug(f"Updated {sku}: {packs} packs, {multiplier} multiplier")
